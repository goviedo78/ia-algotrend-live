-- Auto-approve risk parameter edits for connections that were already approved by an admin
-- Admin approval is required only ONCE during initial setup (approved_at IS NULL).
-- Subsequent edits to lot size / capital / risk parameters automatically apply and set status to ACTIVE.
...
-- Backfill / auto-approve any connection that was already approved by admin previously
-- but was put in PENDING_APPROVAL status due to risk edits:
update public.broker_risk_policies
set enabled = true,
    fixed_notional_usd = coalesce(nullif(suggested_notional_per_order_usd, 0), fixed_notional_usd, 5),
    max_notional_per_order_usd = coalesce(nullif(suggested_notional_per_order_usd, 0), max_notional_per_order_usd, 5),
    max_total_exposure_usd = coalesce(nullif(suggested_max_total_exposure_usd, 0), max_total_exposure_usd, 10),
    daily_loss_limit_usd = coalesce(nullif(suggested_daily_loss_limit_usd, 0), daily_loss_limit_usd, 2),
    min_available_margin_usd = coalesce(nullif(suggested_min_available_margin_usd, 0), min_available_margin_usd, 60),
    allowed_symbols = array[c.requested_symbol],
    updated_at = now()
from public.broker_connections c
where c.id = public.broker_risk_policies.connection_id
...
set enabled = true,
    updated_at = now()
from public.broker_connections c
where c.id = public.broker_strategy_bindings.connection_id
  and c.approved_at is not null
  and c.status = 'PENDING_APPROVAL';
