            ...
            {riskEdit && (() => {
              ...
              return <section className={styles.section} id="editar-capital">
                ...
                <form className={styles.formGrid} onSubmit={requestRiskChange}>
                  ...
                  <label className={`${styles.field} ${privacyMode ? styles.sensitiveInput : ''}`}><BrokerFieldLabel label="Lotaje / Notional por orden (USD)" tooltip="Monto en dólares por orden en cada entrada. El número que quieras: el único límite real es el margen disponible en tu cuenta de BingX. Sólo aplica con interés compuesto apagado." example="Ejemplo: 10.00 USD." /><input type="number" min="1" max="10000000" step="0.01" value={riskEdit.fixedNotionalUsd} onChange={(event) => setRiskEdit((current) => current ? { ...current, fixedNotionalUsd: Number(event.target.value) || 0 } : current)} required /></label>
